...
const int DATA_PIN_4313 = 3;
const int IRQ_PIN_4313 =  2;
...
void loop() {
  char msg[64];
  static uint32_t last_signal_timestamp;
  ...
  if (keyboard.available()) {
    ...
    sprintf(msg, "Sending: %c (%02x)", c, c);
    Serial.println(msg);
//    sendChar(c);
  } else {
    if (current_timestamp - last_signal_timestamp > 5000) {
      last_signal_timestamp = current_timestamp;
...
bool testConnection() {
  ...
  do {
    ...
    if (c == 0xee) {
      ...
      sprintf(msg, "Returned NOK: %02X", c);
      Serial.println(msg);
      return false;
    }
    current = millis();
...
void sendToKeyboard(uint8_t command) {
  ...
  sprintf(msg, "CMD: %d %d%d%d%d%d%d%d%d %d %d", commandBuffer[0], commandBuffer[1], commandBuffer[2], commandBuffer[3],
               commandBuffer[4], commandBuffer[5], commandBuffer[6], commandBuffer[7], commandBuffer[8], commandBuffer[9], commandBuffer[10]);
  Serial.println(msg);
